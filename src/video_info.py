...
def _videos(config: InputOutputConfig, unfiltered: bool = False) -> List[Video]:
    """
    Helper function to get all the videos'video info in the input directory.

    Args:
        config (InputOutputConfig):  The config object
        unfiltered (bool, optional): Whether to return all videos or only videos that have not been processed yet. Defaults to False.
